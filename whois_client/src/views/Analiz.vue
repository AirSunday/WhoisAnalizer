        ...
        <table class="radioAll">
          <td>
            <button
              class="radio1"
              v-bind:class="{
                StatusRadioActiv: radioStatus == 1,
                StatusRadioNoActiv: !radioStatus != 2,
              }"
              @click="
          ...
          <td>
            <button
              class="radio2"
              v-bind:class="{
                StatusRadioActiv: radioStatus == 2,
                StatusRadioNoActiv: radioStatus != 2,
              }"
              @click="
                this.PageDomainInAnaliz = 1;
          ...
          <td>
            <button
              class="radio3"
              v-bind:class="{
                StatusRadioActiv: radioStatus == 3,
                StatusRadioNoActiv: !radioStatus != 2,
              }"
              @click="
                this.PageDomainInAnaliz = 1;
    ...
    <div class="AnalizMain">
      <table v-if="radioStatus == 1" class="TableAnaliz">
        ...
        <tr v-for="(domainA, key) in domainsAnaliz10" :key="key">
          <td class="domenNameAnaliz" @click="copy(domainA.domain_name)">
            {{ domainA.domain_name }}
          </td>
          <td>{{ domainA.age }}</td>
          ...
          <td>
            <p
              v-for="(nsServer, key) in domainA.ns_servers.split(' ')"
              :key="key"
            >
              {{ nsServer }}
            </p>
...
export default {
  ...
  methods: {
    GoPage(direction) {
      if (direction === 1 && this.PageDomainInAnaliz < this.CountPage)
        this.PageDomainInAnaliz += 1;
      else if (direction === -1 && this.PageDomainInAnaliz > 1)
        this.PageDomainInAnaliz -= 1;
    ...
    SortByData() {
      WhoisDataService.GetCountDomain("domain").then((response) => {
        this.CountPage = Math.ceil(response.data[0].reg_count / 5);
      });
      this.radioStatus = 1;
      WhoisDataService.get10(this.PageDomainInAnaliz).then((response) => {
        this.domainsAnaliz10 = response.data;
      });
    },
    SortByNs() {
      this.radioStatus = 2;
      WhoisDataService.GetCountDomain("nsservers").then((response) => {
        this.CountPage = Math.ceil(response.data[0].reg_count / 10);
      });
      WhoisDataService.GetNsServers(this.PageDomainInAnaliz).then(
        (response) => {
    ...
    SortByReg() {
      this.radioStatus = 3;
      WhoisDataService.GetCountDomain("registrant").then((response) => {
        this.CountPage = Math.ceil(response.data[0].reg_count / 10);
      });
      WhoisDataService.GetRegistrant(this.PageDomainInAnaliz).then(
        (response) => {
    ...
    copy(domain) {
      navigator.clipboard
        .writeText(domain)
        .then(() => {
          this.$refs.AddAlertMess.AddAlertMess({
            status: true,
            message: this.$t("alert.t_Domain_Add_Buffer"),
          });
        })
        .catch((err) => {
          ...
          this.$refs.AddAlertMess.AddAlertMess({
            status: false,
            message: this.$t("alert.f_Domain_Add_Buffer"),
          });
