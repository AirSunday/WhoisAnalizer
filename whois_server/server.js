...
db.sequelize.sync()
  ...
  cron.schedule('00 21 * * *', () => { // min hore day mounth year (every day 00:00)
  ...
  cron.schedule('00 22 * * *', () => { // min hore day mounth year (every day 02:00)
    if(process.env.FLAG_REQUEST == 'true'){
      require("./app/controllers/whois.controller.js").CompareDomains();
    }
  });

  cron.schedule('50 22 * * *', () => { // min hore day mounth year (every day 02:00)
    if(process.env.FLAG_REQUEST == 'true'){
