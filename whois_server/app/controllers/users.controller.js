...
exports.GetNews = (req, res) => {
  ...
  var arrNews = [];
  Newsdb.findAll({
    attributes: ["news_id", "title", "preview", "updatedAt"],
...
function SendMessage(name, email, domain) {
  transporter.sendMail({
    ...
    html: `Diar ${name}, The domain name <i>${domain}</i> you were tracking has been <strong>released</strong>`,
  });
}
...
exports.CheckUsersDomain = () => {
  Usersdb.findAll({ where: { [Op.not]: { domains: "" } } }).then((response) => {
